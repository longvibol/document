...
public class JwtSecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
		return httpSecurity.csrf(AbstractHttpConfigurer::disable) // (1)
				...
				.authorizeRequests(auth -> auth.mvcMatchers("/authenticate", "/actuator", "/actuator/*").permitAll()
						.antMatchers(HttpMethod.OPTIONS, "/**").permitAll().requestMatchers(PathRequest.toH2Console())
	...
	public UserDetailsService userDetailsService() {
		UserDetails user = User.withUsername("vibol")
				.password("{noop}123")
				.authorities("read")
				.roles("USER").build();
